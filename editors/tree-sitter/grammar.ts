  ...
  rules: {
    ...
    _simple_type: ($) =>
      choice(
        ...
        $.function_type,
        $.negated_type,
        // Soppo: nullable types and enum types
        $.nullable_type,
        $.enum_type,
      ),

    // Soppo: nullable type (?Type)
    nullable_type: ($) => prec(PREC.unary, seq("?", $._type)),

    // Soppo: enum type (type Name enum { ... })
    enum_type: ($) =>
      seq(
        "enum",
        "{",
    ...
    match_statement: ($) =>
      seq(
        ...
        "{",
        repeat(choice($.match_case, $.default_case)),
        "}",
      ),

    match_case: ($) =>
      seq("case", field("pattern", $.pattern), ":", optional($.statement_list)),
    ...
    qualified_pattern: ($) =>
      seq(field("qualifier", $.identifier), ".", field("name", $.identifier)),

    variant_pattern: ($) =>
      seq(
        field("type", choice($.identifier, $.qualified_pattern)),
        optional(
          choice(
            // Tuple destructuring: Some(x)
            seq("(", commaSep(field("binding", $.identifier)), ")"),
    ...
    _expression: ($) =>
      choice(
        ...
        // Soppo: try expression and nil assertion
        $.try_expression,
    ...
    nil_assertion_expression: ($) =>
      prec(
        PREC.primary,
        seq(field("operand", $._expression), ".", "(", "!", "nil", ")"),
      ),
