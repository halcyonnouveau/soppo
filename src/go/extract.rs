...
fn extract_soppo_markers(source: &str, pkg: &mut GoPackage) {
    // Check for //soppo:generated marker with version
    // Format: //soppo:generated v1
    if let Some(line) = source.lines().find(|l| l.contains("//soppo:generated"))
        && let Some(version) = line.strip_prefix("//soppo:generated").map(|s| s.trim())
    {
        if version == crate::codegen::MARKER_VERSION {
            pkg.soppo_generated = true;
        } else {
            eprintln!(
                "warning: {} was generated with marker {} (current: {}), treating as Go module",
                pkg.name,
                version,
                crate::codegen::MARKER_VERSION
            );
        }
    }
...
"#
    ...
    #[test]
    fn test_marker_version_current() {
        let dir = std::env::temp_dir().join("soppo-marker-version-current");
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();

        let file_path = dir.join("current.go");
        let mut file = File::create(&file_path).unwrap();
        writeln!(
            file,
            r#"//soppo:generated v1
package test
...
"#
        ...
        assert!(
            pkg.soppo_generated,
            "Current marker version should be recognised"
        );
    ...
    #[test]
    fn test_marker_version_mismatch() {
        let dir = std::env::temp_dir().join("soppo-marker-version-mismatch");
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();

        let file_path = dir.join("old.go");
        let mut file = File::create(&file_path).unwrap();
        writeln!(
            file,
            r#"//soppo:generated v0
package test
...
"#
        ...
        assert!(
            !pkg.soppo_generated,
            "Mismatched marker version should not be recognised"
        );
